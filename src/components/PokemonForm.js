...
function SortableImage({ id, url, onRemove }) {
    ...
    return (_jsxs("div", { ref: setNodeRef, style: style, ...attributes, className: "relative w-24 h-24", children: [_jsx("img", { src: url, loading: "lazy", className: "w-full h-full object-cover rounded cursor-move", ...listeners }), _jsx("button", { onClick: (e) => {
                    e.stopPropagation();
                    onRemove(id);
...
export default function PokemonForm() {
    ...
    return (_jsxs("div", { className: "flex gap-6", children: [_jsxs("form", { className: "bg-white p-6 rounded shadow-md w-full md:w-1/3", onSubmit: handleSubmit, children: [Object.entries(formData).map(([key, value]) => (_jsxs("div", { className: "mb-4", children: [_jsx("label", { className: "block mb-1 capitalize", children: key }), key === "status" ? (_jsxs("select", { name: key, value: value, onChange: handleChange, className: "w-full border p-2", children: [_jsx("option", { value: "", children: "Select Status" }), [
                                        ...
                                        "Sold",
                                        "Archived",
                                    ].map((status) => (_jsx("option", { children: status }, status)))] })) : key === "notes" ? (_jsx("textarea", { name: key, value: value, onChange: handleChange, className: "w-full border p-2", rows: 3 })) : (_jsx("input", { name: key, type: key.includes("price") ? "number" : "text", step: "0.01", value: value, onChange: handleChange, className: "w-full border p-2" }))] }, key))), _jsxs("div", { className: "mb-4", children: [_jsx("label", { className: "block mb-1", children: "Upload Images" }), _jsx("input", { type: "file", multiple: true, accept: "image/*", onChange: handleImageUpload, className: "w-full border p-2" })] }), _jsx(DndContext, { sensors: sensors, collisionDetection: closestCenter, onDragEnd: handleDragEnd, children: _jsx(SortableContext, { items: items.map((img) => img.id), strategy: verticalListSortingStrategy, children: _jsx("div", { className: "flex flex-wrap gap-2 mb-4", children: items.map((img) => (_jsx(SortableImage, { id: img.id, url: img.url, onRemove: handleImageRemove }, img.id))) }) }) }), _jsxs("button", { className: "bg-blue-600 text-white px-4 py-2 rounded", type: "submit", children: [editingId ? "Update" : "Save", " Pokemon"] })] }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("h2", { className: "text-xl font-semibold mb-4", children: "Pokemon List" }), _jsx("div", { className: "space-y-4", children: pokemonList.map((item) => (_jsxs("div", { className: "bg-white p-4 rounded shadow relative", children: [_jsx("h3", { className: "text-lg font-bold", children: item.name }), _jsxs("p", { className: "text-sm text-gray-600", children: ["Set: ", item.set, " | Condition: ", item.condition] }), _jsxs("p", { className: "text-sm", children: ["Price: $", item.price, " | Purchase: $", item.purchasePrice] }), _jsxs("p", { className: "text-sm", children: ["Status: ", item.status] }), _jsx("div", { className: "flex flex-wrap gap-2 mt-2", children: (item.images || []).map((url, index) => (_jsx("img", { src: url, loading: "lazy", className: "w-16 h-16 object-cover rounded" }, index))) }), _jsxs("div", { className: "mt-2 flex gap-2", children: [_jsx("button", { onClick: () => handleEdit(item), className: "bg-yellow-500 text-white px-3 py-1 rounded", children: "Edit" }), _jsx("button", { onClick: () => handleDelete(item.id), className: "bg-red-600 text-white px-3 py-1 rounded", children: "Delete" })] })] }, item.id))) })] })] }));
}
